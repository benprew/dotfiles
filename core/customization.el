(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(aquamacs-additional-fontsets nil t)
 '(aquamacs-customization-version-id 100 t)
 '(c-basic-offset 4)
 '(case-fold-search t)
 '(column-number-mode t)
 '(compilation-ask-about-save nil)
 '(cperl-auto-newline nil)
 '(cperl-brace-imaginary-offset 0)
 '(cperl-brace-offset 0)
 '(cperl-close-paren-offset -4)
 '(cperl-continued-brace-offset 0)
 '(cperl-continued-statement-offset 0)
 '(cperl-font-lock t)
 '(cperl-indent-level 4)
 '(cperl-indent-parens-as-block t)
 '(cperl-label-offset -4)
 '(cperl-merge-trailing-else nil)
 '(cperl-tab-always-indent t)
 '(cperl-under-as-char t)
 '(cua-auto-tabify-rectangles nil)
 '(cua-enable-cua-keys nil)
 '(cua-enable-cursor-indications t)
 '(cua-normal-cursor-color (quote (box . "red")))
 '(current-language-environment "UTF-8")
 '(default-input-method "latin-1-prefix")
 '(global-auto-revert-mode t)
 '(grep-find-command "\"ack --nogroup --nocolor -H \"")
 '(indent-tabs-mode nil)
 '(inhibit-startup-screen t)
 '(linum-format (quote "%6d "))
 '(magit-diff-options nil)
 '(make-backup-files nil)
 '(menu-bar-mode t)
 '(midnight-mode t nil (midnight))
 '(ns-alternate-modifier nil)
 '(ns-command-modifier (quote meta))
 '(org-agenda-files (quote ("~/notes/kw_control.org" "~/notes/1.44.org")))
 '(org-log-done (quote time))
 '(package-user-dir "~/.emacs.d/elpa.d")
 '(printer-name "violet")
 '(ps-lpr-command "myprint.sh")
 '(ps-lpr-switches (quote ("")))
 '(ps-n-up-printing 2)
 '(ps-printer-name "")
 '(ps-printer-name-option "-P")
 '(query-user-mail-address nil)
 '(ruby-deep-arglist nil)
 '(ruby-deep-indent-paren nil)
 '(scroll-bar-mode nil)
 '(scroll-conservatively 100)
 '(scroll-margin 2)
 '(show-paren-mode t)
 '(size-indication-mode t)
 '(tab-width 4)
 '(term-bind-key-alist
   (quote
    (("C-c C-c" . term-interrupt-subjob)
     ("C-p" . previous-line)
     ("C-n" . next-line)
     ("C-s" . isearch-forward)
     ("C-r" . isearch-backward)
     ("C-m" . term-send-raw)
     ("M-f" . term-send-forward-word)
     ("M-b" . term-send-backward-word)
     ("M-o" . term-send-backspace)
     ("M-p" . term-send-up)
     ("M-n" . term-send-down)
     ("M-M" . term-send-forward-kill-word)
     ("M-N" . term-send-backward-kill-word)
     ("M-r" . term-send-reverse-search-history)
     ("M-," . term-send-input)
     ("M-." . comint-dynamic-complete))))
 '(tool-bar-mode nil)
 '(vc-diff-switches (quote ("-U500")))
 '(virtualenv-root "~/Envs")
 '(virtualenv-root-dir "/Users/benprew/Envs/")
 '(visible-bell t))


(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(helm-selection ((t (:background "controlDarkShadowColor" :underline t)))))

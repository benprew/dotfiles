...
ENCRYPTION_PIPE = ["gpg", "--encrypt", "--recipient", "ben@throwingbones.com"]
...
if args.authorize:
    ...
    if authflow in ("authcode", "localhostauthcode"):
        ...
        try:
            response = urllib.request.urlopen(
                registration["devicecode_endpoint"], urllib.parse.urlencode(p).encode()
            )
        except urllib.error.HTTPError as err:
            print(err.code, err.reason)
            response = err
...
if args.test:
    ...
    sasl_string = build_sasl_string(
        token["email"], registration["imap_endpoint"], 993, token["access_token"]
    )
    if args.debug:
        imap_conn.debug = 4
    try:
        imap_conn.authenticate(
            registration["sasl_method"], lambda _: sasl_string.encode()
        )
    ...
    sasl_string = build_sasl_string(
        token["email"], registration["pop_endpoint"], 995, token["access_token"]
    )
    if args.debug:
    ...
    sasl_string = build_sasl_string(
        token["email"], registration["smtp_endpoint"], 587, token["access_token"]
    )
    smtp_conn.ehlo("test")
    smtp_conn.starttls()
    smtp_conn.ehlo("test")
